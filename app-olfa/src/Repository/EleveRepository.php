...
class EleveRepository extends ServiceEntityRepository
{
    ...
    */
    ...
    public function getEleveByKeyword($value)
    {
        return $this->createQueryBuilder('e')
            ->where('e.nom = :nom')
            ->setParameter('nom', $value)
            ->orWhere('e.prenom = :prenom')
            ->setParameter('prenom', $value)
            ->orWhere('e.cinParent = :cinParent')
            ->setParameter('cinParent', $value)
            ->orWhere('e.matricule = :matricule')
            ->setParameter('matricule', $value)
            ->getQuery()
            ->getResult()
        ;
    }
